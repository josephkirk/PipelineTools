...
def setCommand():
    ...
    #pm.nameCommand('HairOpsMarkingMenu_PressNameCommand',ann='Hai',c=command)
    #pm.hotkey(k = '`',alt = True, n = 'HairOpsMarkingMenu_Press', rn = 'HairOpsMarkingMenu_Release')
    ...
    for category in commandsDict:
        for command in commandsDict[category]:
            ...
            nameCommand = pm.nameCommand(command+"NameCommand",ann=commandsDict[category][command][1],c=command)
            nameCommandList.append((nameCommand,commandsDict[category][command][2]))
