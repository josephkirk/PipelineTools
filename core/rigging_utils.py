...
def createCircle():
    ...
    while theta<=2*math.pi:
        ...
        x = offset[0] + radius*math.cos(theta)
        y = offset[1] + radius*math.sin(theta)
        pointMatrix.append([0,x,y])
        theta+=inc
    print pointMatrix
    key = range(len(pointMatrix))
    pm.curve(d=1,p=pointMatrix,k=key)
class ControlObject(object):
    '''Class to create Control'''
    def __init__(self, name='ctl', radius=0.5 ,color=pm.dt.Color.red, length=2.0, axis='Y'):
        self.name = name
        self.radius = radius
        self.length = length
        self.axis = axis
    ...
    def __setProperty__(func):
        ...
        def wrapper(self,*args, **kws):
            ...
            control = func(self)
            #control.rename(func.__name__+'_'+self.name)
            self.controls.append(control)
            self.setColor(control=control)
            self.setAxis(control)
            print(control,'type',func.__name__,'created along',self.axis)
            if kws.has_key('group'):
                Gp = self.group(control)
                self.controlGps.append(Gp)
            if args:
                xformOb = Gp if kws.has_key('group') else control
                self._Xform(xformOb, args[0])
            return control
        return wrapper
    #### Control Type
    @__setProperty__
    def Octa(self):
        rd = self.radius
        pntMatrix = [
            [ 0, rd, 0], [ rd, 0, 0], [ 0, 0,-rd],
            [ 0, rd, 0],	[-rd, 0, 0], [ 0, 0,-rd],
            [ 0,-rd, 0],	[ rd, 0, 0],	[ 0, 0, rd],
            [-rd, 0, 0],	[ 0,-rd, 0],	[ 0, 0, rd], [ 0, rd, 0]]
        crv = pm.curve(d=1, p=pntMatrix, n=self.name)
        return crv
    ...
    @__setProperty__
    def Circle(self):
        crv = pm.circle(nr=[1,0,0], r=self.radius, n=self.name, ch=False)[0]
        return crv
    @__setProperty__
    def Sphere(self):
        pntMatrix = [
            ...
            [0, 0, -1]]
        key = range(53)
        crv = pm.curve(d=1,k=key, p=pntMatrix, n=self.name)
        return crv
    #### control method
    ...
    def setColor(self, color=[], control=None):
        if any([isinstance(color,condition) for condition in [tuple,set,list,dict,pm.dt.Color]]):
            if len(color)>=3:
                self.color = color
        if control and isinstance(control,pm.nt.DagNode):
    ...
    def group(ob):
        ...
        return Gp
    @classmethod
    def createControlOn(cls,joints,type='Pin',parentLast=False, connect=True,atrConnect={'translate':True,'rotate':True,'scale':False}, **kwargs):
        controller = cls(**kwargs)
        controlType = {
            'Pin':controller.Pin,
            'Circle':controller.Circle,
            'Octa':controller.Octa,
            'Sphere':controller.Sphere
        }
        for joint in joints:
            pm.select(cl=True)
            createOffsetJoint(joint)
            controlname = joint.name().split('|')[-1].split('_')[0]+'_ctl'
            controlfunc = ul.partial(controlType[type], name=controlname, group=True)
            control = controlfunc()
            Gp = control.getParent()
            pm.select(cl=True)
            xformTo(Gp, joint)
            if len(controller.controls)>1 and parentLast:
                Gp.setParent(controller.controls[-2])
            pm.select(cl=True)
            connectTransform(control,joint,**atrConnect)
        return controller
